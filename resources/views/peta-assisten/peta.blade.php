		...
		<div class="panel panel-default">
			...
			<div class="row">
				<div class="col-md-3 col-md-offset-4" style="padding: 10px 10px 10px 10px;text-align:center; margin-bottom:10px;color:black;background:rgba(194, 214, 214,0.3);">
				    @if(!\App\Taksasi::where('id_taksasi',\App\Taksasi::max('id_taksasi'))->pluck('tgl_taksasi')->first())
				        <strong>Tanggal Taksasi : {{ date('d/m/Y') }}</strong>
				    </div>
				    @else
				        <strong>Tanggal Taksasi : {{ date('d/m/Y',strtotime(\App\Taksasi::where('id_taksasi',\App\Taksasi::max('id_taksasi'))->pluck('tgl_taksasi')->first())) }}</strong>
				    </div>
				    @endif
				...
				<div class="card" style="width: 18rem;">
					<div class="card-body">
						<table style="margin-top:12px" class="table table-bordered">
							...
							<tbody>
								<tr>
									<td style="text-align:center"><h3><strong>{{ number_format( $tahunanbuah, 0 , '.' , ',' ) }}</strong></h3> buah</td>
									<td style="text-align:center"><h3><strong>{{ number_format( $tahunanberat, 0 , '.' , ',' ) }}</strong></h3> Kg</td>
								</tr>
							</tbody>
						</table>
					</div>
				...
				<div class="card" style="width: 18rem;">
					<div class="card-body">
						<table style="margin-top:12px" class="table table-bordered">
							...
							<tbody>
								<tr>
									<td style="text-align:center"><h3><strong>{{ number_format( $bulananbuah, 0 , '.' , ',' ) }}</strong></h3> buah</td>
									<td style="text-align:center"><h3><strong>{{ number_format( $bulananberat, 0 , '.' , ',' ) }}</strong></h3> Kg</td>
								</tr>
							</tbody>
						</table>
					</div>
				...
				<div class="card" style="width: 18rem;">
					<div class="card-body">
						<table style="margin-top:12px" class="table table-bordered">
							...
							<tbody>
								<tr>
									<td style="text-align:center"><h3><strong>{{ number_format( $mingguanbuah, 0 , '.' , ',' ) }}</strong></h3> buah</td>
									<td style="text-align:center"><h3><strong>{{ number_format( $mingguanberat, 0 , '.' , ',' ) }}</strong></h3> Kg</td>
								</tr>
							</tbody>
						</table>
					</div>
				...
				<div class="card" style="width: 18rem;">
					<div class="card-body">
						<table style="margin-top:12px" class="table table-bordered">
							...
							<tbody>
								<tr>
									<td style="text-align:center"><h3><strong>{{ number_format( $harianbuah, 0 , '.' , ',' ) }}</strong></h3> buah</td>
									<td style="text-align:center"><h3><strong>{{ number_format( $harianberat, 0 , '.' , ',' ) }}</strong></h3> Kg</td>
								</tr>
							</tbody>
						</table>
					</div>
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Tambah Taksasi: </h4>
			...
			<form method="post" action="{{ route('tambahtaksasi') }}">
				{{ csrf_field() }}
	...
	function initMap() {
		...
		<?php foreach(\App\Blok::all() as $blok): ?>
			<?php if(\App\Afdeling::where('id_afdeling',$blok->id_afdeling)->pluck('id_user')->first() == \Auth::user()->id_user):?>
				var wilayah = IO.OUT((JSON.parse(<?php echo json_encode($blok->koordinat)?>)),map,'{{ $afd->warna }}');
				var shape = wilayah[0];
				var infowindow = new google.maps.InfoWindow();
				...
				google.maps.event.addListener(shape, 'click', function(e) {
					...
					infowindow.setContent(
						...
						"<table class='table'>"+
						"<tr>"+
						"<td>Nama Krani</td><td>:</td><td>{{ \App\User::where('id_user',\App\Krani::where('id_blok',$blok->id_blok)->pluck('id_user')->first())->pluck('nama')->first() }}</td>"+
						"</tr>"+
						"<tr>"+
						"<td>ID Afdeling</td><td>:</td><td>{{ $blok->id_afdeling }}</td>"+
						"</tr>"+
						"<tr>"+
						"<td>ID Blok</td><td>:</td><td>{{ $blok->id_blok }}</td>"+
						"</tr>"+
						"<tr>"+
						"<td>Deskripsi</td><td>:</td><td>{{ $blok->deskripsi }}</td>"+
						"</tr>"+
						"<tr>"+
						"<td>Jumlah Pohon</td><td>:</td><td>{{ number_format( $blok->jumlah_pohon, 0 , '.' , ',' ) }}</td>"+
						"</tr>"+
						"<tr>"+
						"<td>Luas Blok</td>"+
						...
						"<tr>"+
						"<td>Jumlah Panen</td>"+
						"<td>:</td><td>{{ number_format( \App\Taksasi::where('id_blok',$blok->id_blok)->pluck('panen')->first(), 0 , '.' , ',' ) }} Buah</td></tr>"+
						"</table>"+
						"<td></td>"+
						"<td>:</td><td><button class='btn btn-sm btn-primary inputtaksasi' data-idafdeling='{{ $blok->id_afdeling }}' data-idblok='{{ $blok->id_blok }}' data-toggle='modal' data-target='#modaltaksasi'>Tambah Taksasi</button></td></tr>"+
						"</table>");
					infowindow.open(map);
				});
			<?php endif; ?>
