	...
	@endif
            ...
            <button class="tbhtphshow btn btn-primary btn-md btn-block">Menu Tambah TPH</button>
				...
				<div class="menuhapus" style="display:none">
					...
					<h3 style="margin-left:15px">Hapus Peta Afdeling</h3>
						<form id="hapusafdeling" action="{{ route('deleteafdeling') }}" method="post">
							{{ csrf_field() }}
							{{ method_field('delete') }}
							<div class="form-group col-md-3">
							<label>Pilih Afdeling</label>
									<select class="form-control afd-pilih-1" name="afd">
										@foreach(\App\Afdeling::all() as $af)
											<option value="{{ $af->id_afdeling }}">{{ $af->deskripsi }}</option>
										@endforeach
									</select>
							</div>
							<br/>
					...
					<h3 style="margin-left:15px">Hapus Peta Blok</h3>
						<form id="hapusblok" action="{{ route('hapusblok') }}" method="post">
							{{ csrf_field() }}
							{{ method_field('delete') }}
							<div class="form-group col-md-3">
							<label>Pilih Afdeling</label>
							...
							<label>Pilih Blok</label>
								<div id="displaybloklist">
									<select class="form-control" name="blok">
									  @foreach(\App\Blok::where('id_afdeling',\App\Afdeling::min('id_afdeling'))->get() as $blok)
					...
					<div class="row">
						...
						<form id="hapustph" action="{{ route('deletetph') }}" method="post">
							{{ csrf_field() }}
							{{ method_field('delete') }}
							<div class="form-group col-md-3">
							<label>Pilih Afdeling</label>
							...
							<label>Pilih TPH</label>
								<div id="displaytphlist">
									<select class="form-control" name="id">
									  @foreach(\App\TPH::where('id_blok',\App\Blok::min('id_blok'))->get() as $blok)
									    <option value="{{ $blok->tph }}">{{ $blok->deskripsi }}</option>
									  @endforeach
									</select>
								</div>
							</div>
        ...
        <!-- Tambah Afdeling -->
				...
				<div class="menutbhafdeling" style="display:none">
					<div class="row">
						...
						<form id="formtambahpeta" action="{{ route('afdeling.simpan') }}" method="post">
						{{ csrf_field() }}
						...
						<div class="col-md-4 form-group">
							...
							<select class="form-control" name="pengelola">
								...
								@foreach(\App\User::where("id_role",3)->orWhere("id_role",2)->get() as $ast)
									<option value="{{ $ast->id_user }}">{{ $ast->username }}</option>
								@endforeach
							</select>
						</div>
        ...
        <!-- Tambah Blok -->
				...
				<div class="menutbhblok" style="display:none">
					<div class="row">
						...
						<form id="formtambahpetablok" action="{{ route('tambahblok') }}" method="post">
						{{ csrf_field() }}
						<div class="col-md-12 form-group">
						<div class="col-md-3">
							<label>Pilih Afdeling</label>
							<select class="form-control" name="afdeling">
								...
								@foreach(\App\Afdeling::all() as $af)
									<option style="color:{{ $af->warna }}" value="{{ $af->id_afdeling }}">id : {{ $af->deskripsi }}</option>
								@endforeach
							</select>
						</div>
						<div class="col-md-12 form-group">
						...
						<div class="col-md-12 form-group">
							...
							<select class="form-control" name="pengelola">
								...
								@foreach(\App\User::where("id_role",7)->get() as $ast)
									<option value="{{ $ast->id_user }}">{{ $ast->username }}</option>
								@endforeach
							</select>
						</div>
        ...
        <div class="menutbhtph" style="display:none">
          <div class="row">
            ...
            <form id="formtambahpetatph" action="{{ route('tambahtph') }}" method="post">
            {{ csrf_field() }}
            <div class="col-md-12 form-group">
            <div class="col-md-3">
              <label>Pilih Blok</label>
              <select class="form-control" name="blok">
                ...
                @foreach(\App\Blok::all() as $af)
                  <option value="{{ $af->id_blok }}">id : {{ $af->deskripsi }}</option>
                @endforeach
              </select>
            </div>
            <div class="col-md-12 form-group">
