                ...
                <div class="col-sm-12">
                  <div class="row">
                    <div class="col-md-2" style="background:rgba(102, 153, 153, 0.3); text-align:center">
                      ...
                      <div class="row">
                        <div class="col-md-12">
                          <p><strong style="font-size:50px;">{{ number_format( \App\Kapasitas::where('tgl',date('Y-m-d'))->sum('kapasitas'), 0 , '.' , ',' ) }}</strong><strong> ton/jam</strong></p>
                        </div>
                      </div>
                    </div>
...
var myChart = new Chart(ctx, {
    type: 'bar',
    data: {
        labels: ['{{ date("d-m-Y", strtotime("-4 day")) }}','{{ date("d-m-Y", strtotime("-3 day")) }}','{{ date("d-m-Y", strtotime("-2 day")) }}','{{ date("d-m-Y", strtotime("-1 day")) }}','{{ date("d-m-Y") }}'],
        datasets: [{
            label: "Kapasitas",
            data: ['{{ $sum4 }}', '{{ $sum3 }}', '{{ $sum2 }}', '{{ $sum1 }}', '{{ $sum5 }}'],
            backgroundColor: [
                'rgba(255, 99, 132, 0.2)',
                'rgba(54, 162, 235, 0.2)',
                'rgba(255, 206, 86, 0.2)',
